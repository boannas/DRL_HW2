...
class BaseAlgorithm():
    ...
    def discretize_state(self, obs: dict):
        ...
        """
        # ========= put your code here =========#
        # obs_array = np.array(list(obs['policy'][0]))
        # discrete_obs = tuple((obs_array * self.discretize_state_weight).astype(int))

        # obs_array = (obs['policy'][0])
        # discrete_obs = []
        # for i in range(len(obs_array)):
        #     discrete_obs.append(int(obs_array[i].item() * self.discretize_state_weight[i]))
        # return tuple(discrete_obs)
        obs_array = (obs['policy'][0])
        pos_cart_min, pos_cart_max = -3, 3
        pos_pole_min, pos_pole_max = np.deg2rad(-24), np.deg2rad(24)
        vel_cart_min, vel_cart_max = -3, 3
        vel_pole_min, vel_pole_max = -10, 10
        pos_cart_step_size = (pos_cart_max - pos_cart_min) / self.discretize_state_weight[0]
        pos_pole_step_size = (pos_pole_max - pos_pole_min) / self.discretize_state_weight[1]
        vel_cart_step_size = (vel_cart_max - vel_cart_min) / self.discretize_state_weight[2]
        vel_pole_step_size = (vel_pole_max - vel_pole_min) / self.discretize_state_weight[3]
        pos_cart_dis = int((obs_array[0].item() - pos_cart_min) / pos_cart_step_size)
        pos_pole_dis = int((obs_array[1].item() - pos_pole_min) / pos_pole_step_size)
        vel_cart_dis = int((obs_array[2].item() - vel_cart_min) / vel_cart_step_size)
        vel_pole_dis = int((obs_array[3].item() - vel_pole_min) / vel_pole_step_size)
        return tuple([pos_cart_dis, pos_pole_dis, vel_cart_dis, vel_pole_dis])
        # ======================================#
    ...
    def get_discretize_action(self, obs_dis) -> int:
        ...
        # return torch.argmax(self.q_values[obs_dis]).item()
        return torch.argmax(torch.tensor(self.q_values[obs_dis]))  # Exploitation
        # ======================================#
    ...
    def mapping_action(self, action):
        ...
        return torch.tensor([[min_action + (max_action - min_action) * action / (self.num_of_action - 1)]])
        # ======================================#s
